...
public class Util {
	...
	// 멘토링등록페이지 파일용 메서드
	public static void saveMentoringFile(MultipartFile mf1, String admindir, String userdir) {
		byte[] data;
		
		String mtrimgname = mf1.getOriginalFilename();
		
		try {
			// 멘토 프로필 이미지 파일 저장
			data = mf1.getBytes();
			FileOutputStream fo11 = new FileOutputStream(admindir + mtrimgname);
			fo11.write(data);
			fo11.close();
			FileOutputStream fo12 = new FileOutputStream(userdir + mtrimgname);
			fo12.write(data);
			fo12.close();
