...
class InsertPurchase {
	...
	void contextLoads() {
		//결제완료 버튼을 눌렀을 때
		PurchaseDTO purchase = new PurchaseDTO(0, "qkrtjdgns1234", 20000, null, "네이버페이", "현대카드",202, "공부법 상담 해드립니다.", null,null, null, "09:00", null,3 ,408,100,0);
		try {
			//구매 내용을 등록하고
			...
			PurchaseDetailDTO detail = new PurchaseDetailDTO(0,purchase.getMentoringoption_mentoringoptionid(), r, 0, "x", purchase.getPurdate(), 
					...
					mentoring.getMentorurl(), purchase.getMentoring_mplace(),0 , mentoring.getMcaring(), mentoring.getMentorid(),0, mentoring.getMentor_mentorimg(), null, null, 0);//membercount부분은 member부분에서 저장되기 때문에 굳이 detail에서 넣어줄 이유가 없음
